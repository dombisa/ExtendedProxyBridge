...
// Requires: a same-named .dll (see load_dll()) + WinDivert in same directory, Administrator rights.
...
typedef struct {
    ...
    char     dns_resolution[64]; // "DnsResolution": "proxy;custom;system" (Extended ProxyBridge). "" = no DNS interception (default).
} PBRule;

typedef struct {
    int         localhost_via_proxy;
    int         traffic_logging;
    int         health_check_enabled;    // "HealthCheckEnabled" (Extended ProxyBridge)
    uint32_t    health_check_interval;   // "HealthCheckIntervalSeconds", default 30
    char        custom_dns_servers[512]; // "CustomDnsServers", e.g. "94.140.14.14:53;9.9.9.9:53" (Extended ProxyBridge)
...
static void show_help(const char* prog)
{
    ...
    printf("  --install-service    Install as a Windows service (requires --profile)\n");
    printf("  --uninstall-service  Stop and remove the service\n");
    printf("  --start-service      Start the installed service\n");
    printf("  --stop-service       Stop the running service\n");
    printf("  --service-name <name>         Custom internal + display name (default: ProxyBridgeExtended)\n");
    printf("  --service-description <text>  Custom description shown in services.msc\n");
    printf("                                 NOTE: pass the same --service-name to every\n");
    printf("                                 subsequent command (start/stop/uninstall) if\n");
    printf("                                 you used a custom one at install time.\n\n");
    printf("Examples:\n");
    ...
    printf("  ProxyBridge_CLI.exe --install-service --profile C:\\configs\\prod.pbprofile\n");
    printf("  ProxyBridge_CLI.exe --start-service\n");
    printf("  ProxyBridge_CLI.exe --install-service --profile C:\\configs\\prod.pbprofile ^\n");
    printf("      --service-name \"MyCompanyProxy\" --service-description \"Company outbound proxy router\"\n");
    printf("  ProxyBridge_CLI.exe --start-service --service-name \"MyCompanyProxy\"\n\n");
}
...
static bool load_profile(const char* path, PBProfile* prof)
{
    ...
    prof->health_check_enabled  = jbool(json, "HealthCheckEnabled", false) ? 1 : 0;
    prof->health_check_interval = (uint32_t)jint(json, "HealthCheckIntervalSeconds", 30);
    jstr(json, "CustomDnsServers", prof->custom_dns_servers, sizeof(prof->custom_dns_servers));
    ...
    if (arr)
    {
               ...
               prof->num_rules < MAX_RULES)
        {
            ...
            jstr(buf, "DnsResolution", r->dns_resolution, sizeof(r->dns_resolution));
    ...
    do {                                                                          \
        ...
        if (!(var)) {                                                             \
            fprintf(stderr, "ERROR: '%s' not found in %s\n",                     \
                    (name), g_dll_display_name);                                  \
            return false;                                                         \
        }                                                                         \
    } while (0)

// Applies the profile's failover settings (Extended ProxyBridge) using the same
// IdMap that AddProxyConfig's loop just built: translates each config's
// FallbackConfigId (a profile "Id") to the real DLL-assigned id, links it via
// ProxyBridge_SetProxyFallback, and turns on the health-check thread if the
// profile asked for it. Silently does nothing if g_SetFallback/g_SetHealthCheck
// are NULL, i.e. the loaded DLL is a plain upstream build without failover.
typedef struct { uint32_t profile_id; uint32_t dll_id; } IdMap;

static void apply_failover_config(const PBProfile* prof, const IdMap* id_map, int id_map_n)
{
    ...
    for (int i = 0; i < prof->num_configs; i++)
    {
        ...
        {
            if (id_map[j].profile_id == c->profile_id)        from_dll_id = id_map[j].dll_id;
            if (id_map[j].profile_id == c->fallback_profile_id) to_dll_id   = id_map[j].dll_id;
        }
        if (from_dll_id != 0 && to_dll_id != 0)
            g_SetFallback(from_dll_id, to_dll_id);
    }

    if (prof->health_check_enabled)
        g_SetHealthCheck(TRUE, prof->health_check_interval);
}
...
static bool load_dll(void)
{
    ...
    char* dot = strrchr(dll_path, '.');

    if (dot && (!sep || dot > sep))
        *dot = '\0'; // strip ".exe" (or whatever extension) off the copy
    // (if there's genuinely no extension, dll_path is already just the exe's
    // full path with nothing to strip - appending ".dll" below still works)

    strncat_s(dll_path, MAX_PATH, ".dll", _TRUNCATE);

    // Keep just the filename for display/error messages.
    const char* base = strrchr(dll_path, '\\');
    strncpy_s(g_dll_display_name, MAX_PATH, base ? base + 1 : dll_path, _TRUNCATE);
    ...
    if (!g_hDll)
    {
        fprintf(stderr, "ERROR: Cannot load %s (error %lu)\n",
                g_dll_display_name, GetLastError());
        fprintf(stderr, "Ensure %s is in the same directory as this executable,\n", g_dll_display_name);
        fprintf(stderr, "and is named to match it (rename both files together).\n");
        return false;
    }
    ...
    // Extended ProxyBridge failover functions: optional, only present in the
...
// Defaults; overridable at install time via --service-name / --service-description.
// Whatever name was used at --install-service MUST be passed again (via the
// same flags) to --start-service / --stop-service / --uninstall-service, since
// SCM has no way to "discover" which custom name we picked - it just looks up
// services by exact name, like any other Windows service.
static char g_svc_name[256]        = "ProxyBridgeExtended";
static char g_svc_display_name[256] = "Extended ProxyBridge Service";
static char g_svc_description[512] = "Runs Extended ProxyBridge headless as a Windows service.";

static SERVICE_STATUS        g_SvcStatus;
static SERVICE_STATUS_HANDLE g_SvcStatusHandle = NULL;
static HANDLE                g_SvcStopEvent    = NULL;
static char                  g_SvcProfilePath[MAX_PATH] = {0};
static int                   g_SvcVerbose      = 0;

static void ReportSvcStatus(DWORD state, DWORD exitCode, DWORD waitHintMs)
{
...
static VOID WINAPI SvcMain(DWORD argc, LPSTR* argv)
{
    ...
    g_SvcStatusHandle = RegisterServiceCtrlHandlerExA(g_svc_name, SvcCtrlHandlerEx, NULL);
    ...
    g_SvcStopEvent = CreateEventA(NULL, TRUE, FALSE, NULL);

    PBProfile prof;
    if (!g_SvcProfilePath[0] || !load_profile(g_SvcProfilePath, &prof) || prof.num_rules == 0)
    {
    ...
    if (!load_dll())
    {
        ReportSvcStatus(SERVICE_STOPPED, ERROR_DLL_INIT_FAILED, 0);
        return;
    }

    if (g_SvcVerbose == 1 || g_SvcVerbose == 3) g_SetLog(log_cb);
    if (g_SvcVerbose == 2 || g_SvcVerbose == 3) g_SetConn(conn_cb);
    ...
    for (int i = 0; i < prof.num_configs; i++)
    {
        ...
        uint32_t did = g_AddProxyConfig(c->type, c->host, (uint16_t)c->port, c->username, c->password);
        if (did == 0) continue;
    ...
    apply_failover_config(&prof, id_map, id_map_n);
    ...
    for (int i = 0; i < prof.num_rules; i++)
    {
        ...
        {
            for (int j = 0; j < id_map_n; j++)
                if (id_map[j].profile_id == r->proxy_config_id) { dll_cfg_id = id_map[j].dll_id; break; }
            if (dll_cfg_id == 0) dll_cfg_id = id_map[0].dll_id;
        }
        ...
        uint32_t rid = g_AddRule(proc, hosts, ports, domains, r->protocol, r->action, dll_cfg_id);
        if (rid != 0 && !r->is_enabled) g_DisableRule(rid);
        if (rid != 0 && g_SetRuleDns && r->dns_resolution[0]) g_SetRuleDns(rid, r->dns_resolution);
    }
    ...
    while (InterlockedCompareExchange(&g_running, 0, 0) != 0)
        WaitForSingleObject(g_SvcStopEvent, 200);
...
static bool build_service_command_line(char* out, size_t out_size, const char* profile_path, int verbose)
{
    char exe_path[MAX_PATH];
    if (!GetModuleFileNameA(NULL, exe_path, MAX_PATH)) return false;
    int n;
    if (verbose > 0)
        n = snprintf(out, out_size, "\"%s\" --service --service-name \"%s\" --profile \"%s\" --verbose %d",
                     exe_path, g_svc_name, profile_path, verbose);
    else
        n = snprintf(out, out_size, "\"%s\" --service --service-name \"%s\" --profile \"%s\"",
                     exe_path, g_svc_name, profile_path);
    return n > 0 && (size_t)n < out_size;
}

static int install_service(const char* profile_path, int verbose)
{
    if (!profile_path || !profile_path[0])
    {
        fprintf(stderr, "ERROR: --install-service requires --profile <file.pbprofile>.\n");
        return 1;
    }
    char abs_profile[MAX_PATH];
    if (GetFullPathNameA(profile_path, MAX_PATH, abs_profile, NULL) == 0)
    {
        fprintf(stderr, "ERROR: Could not resolve profile path.\n");
        return 1;
    }
    if (GetFileAttributesA(abs_profile) == INVALID_FILE_ATTRIBUTES)
    {
    ...
    char cmdline[1024];
    if (!build_service_command_line(cmdline, sizeof(cmdline), abs_profile, verbose))
    {
    ...
    SC_HANDLE svc = CreateServiceA(
        scm, g_svc_name, g_svc_display_name,
    ...
    if (!svc)
    {
        ...
        if (err == ERROR_SERVICE_EXISTS)
            fprintf(stderr, "ERROR: Service already installed. Use --uninstall-service first.\n");
        else
            fprintf(stderr, "ERROR: CreateService failed (%lu).\n", err);
        return 1;
    }

    SERVICE_DESCRIPTIONA desc = { (LPSTR)g_svc_description };
    ChangeServiceConfig2A(svc, SERVICE_CONFIG_DESCRIPTION, &desc);

    printf("Service '%s' (%s) installed.\n", g_svc_display_name, g_svc_name);
    printf("Profile: %s\n", abs_profile);
    char self_name[MAX_PATH];
    GetModuleFileNameA(NULL, self_name, MAX_PATH);
    char* self_base = strrchr(self_name, '\\');
    self_base = self_base ? self_base + 1 : self_name;

    if (strcmp(g_svc_name, "ProxyBridgeExtended") == 0)
        printf("Start it with: %s --start-service\n", self_base);
    else
        printf("Start it with: %s --start-service --service-name \"%s\"\n", self_base, g_svc_name);
    printf("(or: sc start %s)\n", g_svc_name);
...
static int uninstall_service(void)
{
    ...
    SC_HANDLE svc = OpenServiceA(scm, g_svc_name, SERVICE_STOP | DELETE | SERVICE_QUERY_STATUS);
    if (!svc)
    {
        fprintf(stderr, "ERROR: Service not found (%lu).\n", GetLastError());
        CloseServiceHandle(scm);
        return 1;
    }
...
{
    SC_HANDLE scm = OpenSCManagerA(NULL, NULL, SC_MANAGER_CONNECT);
    if (!scm) { fprintf(stderr, "ERROR: OpenSCManager failed (%lu).\n", GetLastError()); return 1; }
    SC_HANDLE svc = OpenServiceA(scm, g_svc_name, SERVICE_START);
    if (!svc) { fprintf(stderr, "ERROR: Service not found (%lu). Install it first.\n", GetLastError()); CloseServiceHandle(scm); return 1; }
    ...
    {
        printf("Service start requested.\n");
    }
    CloseServiceHandle(svc);
    CloseServiceHandle(scm);
    return 0;
...
static int stop_service_cmd(void)
{
    SC_HANDLE scm = OpenSCManagerA(NULL, NULL, SC_MANAGER_CONNECT);
    if (!scm) { fprintf(stderr, "ERROR: OpenSCManager failed (%lu).\n", GetLastError()); return 1; }
    SC_HANDLE svc = OpenServiceA(scm, g_svc_name, SERVICE_STOP | SERVICE_QUERY_STATUS);
    if (!svc) { fprintf(stderr, "ERROR: Service not found (%lu).\n", GetLastError()); CloseServiceHandle(scm); return 1; }
...
{
    bool do_upd       = false;
    bool svc_mode     = false;   // launched by SCM (internal, not for interactive use)
    bool do_install   = false;
    bool do_uninstall = false;
    bool do_svc_start = false;
    bool do_svc_stop  = false;
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "--start-service") == 0)
        {
            do_svc_start = true;
        }
        else if (strcmp(argv[i], "--stop-service") == 0)
        {
            do_svc_stop = true;
        }
        else if (strcmp(argv[i], "--service-name") == 0 && i + 1 < argc)
        {
            strncpy_s(g_svc_name, sizeof(g_svc_name), argv[++i], _TRUNCATE);
            strncpy_s(g_svc_display_name, sizeof(g_svc_display_name), g_svc_name, _TRUNCATE);
        }
        else if (strcmp(argv[i], "--service-description") == 0 && i + 1 < argc)
        {
            strncpy_s(g_svc_description, sizeof(g_svc_description), argv[++i], _TRUNCATE);
        }
    ...
    // ── Service management commands (each does its own SCM permission check) ──
    if (do_install)   return install_service(profile_path, g_verbose);
    if (do_uninstall) return uninstall_service();
    if (do_svc_start) return start_service_cmd();
    if (do_svc_stop)  return stop_service_cmd();

    // ── Running as a Windows service (launched by SCM, not interactively) ─────
    if (svc_mode)
    {
        if (!profile_path[0])
            return 1; // no console to report to; SCM will just see a fast exit

        strncpy_s(g_SvcProfilePath, MAX_PATH, profile_path, _TRUNCATE);
        g_SvcVerbose = g_verbose;

        SERVICE_TABLE_ENTRYA dispatch_table[] = {
            { (LPSTR)g_svc_name, (LPSERVICE_MAIN_FUNCTIONA)SvcMain },
            { NULL, NULL }
        };
        // Blocks until the service stops; SCM connection fails harmlessly if
        // this exe is ever double-clicked directly instead of started by SCM.
    ...
    // ── Failover: fallback chains + health-check (Extended ProxyBridge) ──────
    apply_failover_config(&prof, id_map, id_map_n);
    if (prof.health_check_enabled)
    {
        if (g_SetHealthCheck)
            printf("\nFailover health check: ON (every %u sec)\n", prof.health_check_interval);
        else
            printf("\nWARNING: Profile requests health check, but this DLL has no failover support (old build?). Ignored.\n");
    }
    ...
    for (int i = 0; i < prof.num_rules; i++)
    {
        ...
        if (g_SetRuleDns && r->dns_resolution[0])
            g_SetRuleDns(rid, r->dns_resolution);

        int p = r->protocol < 3 ? r->protocol : 0;
        int a = r->action    < 3 ? r->action    : 0;
        printf("  [%d] %-28s %-22s %-14s %-20s %s  %s%s%s%s\n",
               i + 1, proc, hosts, ports, domains,
               PROTO[p], ACTION[a],
               r->is_enabled ? "" : "  [disabled]",
               r->dns_resolution[0] ? "  dns=" : "",
               r->dns_resolution[0] ? r->dns_resolution : "");
        ok++;
    }
